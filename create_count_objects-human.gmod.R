...
if (debugMode) {
  opt <- list('organism' = 'hg38', 'experiment' = 'Jlab_exp', 'prefix'='', 'paired' = FALSE,
            'stranded' = FALSE, 'ercc' = FALSE, 'cores' = 4,
            'salmon' = FALSE)
...
cat("Start, mem used: "); pryr:::print.bytes(mem_used())
###
## read in pheno
...
if (opt$salmon) {
    ...
    #######################################################################
    cat(format(Sys.time(), format="[%m/%d %H:%M:%S] "))
    cat("mem used: "); pryr:::print.bytes(mem_used())
    cat("Getting Transcript counts:")
    ...
    cat(format(Sys.time(), format="[%m/%d %H:%M:%S] "))
    cat("txTpm calculated, mem used: "); pryr:::print.bytes(mem_used())
    rm(txMatrices)
}
...
cat("  txMap created, mem used: "); pryr:::print.bytes(mem_used())
...
##-- DEBUG - skip BAM files
 metrics$totalMapped <- metrics$numMapped
 metrics$mitoMapped <- 0
 metrics$mitoRate <- 0
...
#####
cat(format(Sys.time(), format="[%m/%d %H:%M:%S] "))
cat("Gene Counts loaded, mem used: "); pryr:::print.bytes(mem_used())
#####
...
###
cat(format(Sys.time(), format="[%m/%d %H:%M:%S] "))
cat("Exon Counts loaded and exonRpkm created, mem used: "); pryr:::print.bytes(mem_used())
###
...
###
cat(format(Sys.time(), format="[%m/%d %H:%M:%S] "))
cat("Transcript and Exon Maps added, mem used: "); pryr:::print.bytes(mem_used())
###
...
###
cat(format(Sys.time(), format="[%m/%d %H:%M:%S] "))
cat("Transcript and Exon RSE objects saved, mem used: "); pryr:::print.bytes(mem_used())
###
...
###
cat(format(Sys.time(), format="[%m/%d %H:%M:%S] "))
cat("Junction Counts loaded, mem used: "); pryr:::print.bytes(mem_used())
###
...
###
cat(format(Sys.time(), format="[%m/%d %H:%M:%S] "))
cat("Junction Map and RPKMs created, mem used: "); pryr:::print.bytes(mem_used())
###
...
###
cat(format(Sys.time(), format="[%m/%d %H:%M:%S] "))
cat("Done. Junction and Transcript RSE saved, mem used: "); pryr:::print.bytes(mem_used())
###
