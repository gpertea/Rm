...
mlog <- function(msg) {
  cat(format(Sys.time(), format="[%m/%d %H:%M:%S] "))
  cat(msg, "| mem in use: "); pryr:::print.bytes(mem_used())
}
...
#opt <- getopt(spec)
##-- geo test - simulate single input samples

# DEBUG mode: populating the opt object since getopt() does not work here
opt <- list('organism' = 'hg38', 'experiment' = 'Jlab_exp', 'prefix'='', 'paired' = FALSE,
            'stranded' = FALSE, 'ercc' = FALSE, 'cores' = 1,
            'salmon' = FALSE, 'no_biomart' = TRUE)
...
geneCountList = mclapply(geneFn, function(x) {
  cat(".")
  #Matrix(read.delim(pipe(paste("cut -f7", x)), as.is=TRUE,skip=1)[,1])
  #read.delim(pipe(paste("cut -f7", x)), as.is=TRUE,skip=1)[,1]
  read.delim(x, as.is=TRUE, skip=1, colClasses=c(rep("NULL",6), "integer"))[,1]
}, mc.cores=opt$cores)
#restore sample order after parallel processing
geneCountList=geneCountList[metrics$SAMPLE_ID]

geneCounts = do.call("cbind", geneCountList)
##-- are rownames really needed?
#rownames(geneCounts) = rownames(geneMap)
#geneCounts = geneCounts[,metrics$SAMPLE_ID] # put in order - there is no need
...
#restore sample order after parallel processing
exonCountList=exonCountList[metrics$SAMPLE_ID]

exonCounts = do.call("cbind", exonCountList)
###
##rownames(exonCounts) = rownames(exonMap) # not needed?
##exonCounts = exonCounts[,metrics$SAMPLE_ID] # put in order - no need 
...
## import the Junctions annotation
...
#-------- jaffelab function 
junctionCountFn = function (junctionFiles, sampleNames = names(junctionFiles), 
          output = c("Count", "Rail"), minOverhang = 0, 
          strandSpecific = FALSE, illuminaStranded = FALSE, minCount = 1, 
          maxCores = 1) 
{
      ...
      if (output == "Rail") {
        y <- read.delim(x, skip = 1, header = FALSE, 
                        colClasses = c("character", "integer", 
                                       ...
                                       "integer"), col.names = c("chr", 
                                                                 "start", "end", "leftHang", 
                                                                 "rightHang", "count"))
        y <- y[y$count >= minCount, ]
        y <- y[y$leftHang > minOverhang & y$rightHang > 
                 minOverhang, ]
      }
      else if (output == "Count") {
        y <- read.delim(x, skip = 1, header = FALSE, 
                        col.names = c("chr", "start", "end", 
                                      "strand", "count"), colClasses = c("character", 
  ...
  fullGR <- sort(fullGR)
  fullGR$count <- NULL
  message(paste(Sys.time(), "there are", length(fullGR), 
                "total junctions"))
  ...
  for (i in seq(along = grList)) {
    ...
    countList[[i]] <- Rle(cc) # Matrix here?
  }
...
if (any(manifest[,ncol(manifest)] == "unstranded")) {
	juncCounts = junctionCountFn(junctionFiles, metrics$SAMPLE_ID,
