---
title: "create_count_objects-human using sparse matrices"
...
Collect Salmon/Kallisto quantification data
...
if (opt$salmon) {
    ...
    #######################################################################
    mlog("before getting transcript counts (Kallisto)")
...
Gencode annotation parsing, preparing geneMap
...
mlog("Before gene counts")
```
Getting gene counts

```{r}
cat("loading gene counts ")
geneCountList = lapply(geneFn, function(x) {
	cat(".")
	Matrix(read.delim(x, as.is=TRUE, skip=1, colClasses=c(rep("NULL",6), "integer"))[,1], sparse=TRUE)
  #read.delim(x, as.is=TRUE, skip=1, colClasses=c(rep("NULL",6), "integer"))[,1]
})
cat(" done.\n")
#restore sample order after parallel processing? mclapply should preserve the order?
...
##no need to put in order, if geneCountList was already sorted?
#geneCounts = geneCounts[,metrics$SAMPLE_ID] # put in order

## are rownames really needed?
#rownames(sp_geneCounts) = rownames(geneMap)
#no need to put in order, if geneCountList was already sorted?
# -- cannot sort sparse matrix
#sp_geneCounts = sp_geneCounts[,metrics$SAMPLE_ID] # put in order
...
tmetrics <- cbind(metrics, geneStats_t)

# rna Rate
tmetrics$rRNA_rate = colSums(geneCounts[which(geneMap$gene_type == "rRNA"),])/colSums(geneCounts)

# make RPKM
bg = matrix(rep(as.numeric(geneStats["Assigned",])), nc = nrow(tmetrics), 
	          nr = nrow(geneCounts),	byrow=TRUE)
widG = matrix(rep(geneMap$Length), nr = nrow(geneCounts), 
	          nc = nrow(tmetrics),	byrow=FALSE)
geneRpkm = geneCounts/(widG/1000)/(bg/1e6)
#mode(geneCounts@x) <- "integer"

## save metrics
write.csv(tmetrics, file = file.path(".",
    paste0('read_and_alignment_metrics_', EXPNAME,
    '.csv')))

mlog("Gene Counts loaded")
```
Gathering exon counts
...
## counts
mlog("Getting exon counts")

exonCountList = lapply(exonFn, function(x) {
	cat(".")
	#read.delim(x, as.is=TRUE, skip=1, colClasses=c(rep("NULL",6), "integer"))[,1]
	Matrix(read.delim(x, as.is=TRUE, skip=1, colClasses=c(rep("NULL",6), "integer"))[,1])
})
cat(" loaded.\n")
...
## for dcgMatrix we need to restore column names:
#colnames(exonCounts) =  metrics$SAMPLE_ID

## no need to, exonFn is in the right order
...
#mode(exonCounts@x) <- "integer"
###
mlog("Exon Counts loaded")
```
Transcript maps
```{r}
### add transcript maps ####
...
###
mlog("Transcript and Exon Maps added")
```
...
rse_gene <- SummarizedExperiment(assays = list('counts' = geneCounts),
    rowRanges = gr_genes, colData = tmetrics)
...
rse_exon <- SummarizedExperiment(assays = list('counts' = exonCounts),
    rowRanges = gr_exons, colData = tmetrics)
...
mlog("Transcript and Exon RSE objects saved")
### 
```
Junction function from jaffelab package:
```{r}
#-------- jaffelab function 
...
stranded <- !any(manifest[,ncol(manifest)] == "unstranded")
juncCounts = junctionCountFn(junctionFiles, metrics$SAMPLE_ID,
		output = "Count", maxCores=opt$cores,strandSpecific=stranded)
...
############ jRpkm
bgJ = matrix(rep(colSums(jCounts)), nc = nrow(tmetrics), 
...
###
mlog("Junction Map and RPKMs created")
### 
```
...
tosaveCounts = c("tmetrics", "geneCounts", "geneMap", "exonCounts", "exonMap", "jCounts", "jMap", 
					"txNumReads", "txMap" )
tosaveRpkm = c("tmetrics", "geneRpkm", "geneMap", "exonRpkm", "exonMap", "jRpkm", "jMap", 
...
rse_jx <- SummarizedExperiment(assays = list('counts' = jCounts),
    rowRanges = jMap, colData = tmetrics)
...
rse_tx = SummarizedExperiment(
    ...
    colData = tmetrics, rowRanges = txMap)
save(rse_tx, file = paste0('rse_tx_', EXPNAME, '_n', N, '.Rdata'))

###
mlog("Done. Junction and Transcript RSE saved")
###
